...
public sealed partial class UnionTypesGenerator : IIncrementalGenerator
{
    ...
    public const string ExcludeFromCodeCoverageOption = "build_property.UnionTypesGenerator_ExcludeFromCodeCoverage";

    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        AddAttributesSource(context);

        var generatorOptionsProvider = context.AnalyzerConfigOptionsProvider.Select((options, ct) =>
        {
            ct.ThrowIfCancellationRequested();
            bool excludeFromCodeCoverage = true;
            if (options.GlobalOptions.TryGetValue(ExcludeFromCodeCoverageOption, out var optionValue)
                && optionValue.Equals("false", StringComparison.OrdinalIgnoreCase))
            {
                excludeFromCodeCoverage = false;
            }

            return new GeneratorOptions(excludeFromCodeCoverage);
        });
        ...
        var generatorContextProvider = generatorOptionsProvider.Combine(compilationContextProvider)
            .Select((a, ct) => new GeneratorContext(a.Right, a.Left));
        ...
        ProcessValues(context, generatorContextProvider, unionTypeDiagnostics, GenerateUnionType);

        var jsonTypes = unionTypeDiagnostics
            .Where(vd => vd.Diagnostics.Count == 0)
            .Where(vd => vd.Value.GenerateJsonConverter);

        var jsonTypesCombination = jsonTypes.Combine(generatorContextProvider);
        ...
        GenerateConverters(context, generatorContextProvider);
    }
        ...
        SourceProductionContext context)
    {
        ...
        CompilationContext compilationContext = generatorContext.Compilation;
    ...
    private static CompilationUnitSyntax GetCompilationUnit(TypeDeclarationSyntax typeDeclaration,
        string? typeNamespace, CompilationContext context)
    {
        SyntaxTriviaList syntaxTriviaList = GetTriviaList(context);
    ...
    private static void ProcessValues<T>(IncrementalGeneratorInitializationContext context,
        IncrementalValueProvider<GeneratorContext> compilationContextProvider,
        IncrementalValuesProvider<ValueDiagnostics<T>> valueDiagnostics,
        Action<T, GeneratorContext, SourceProductionContext> processAction)
    {
        ...
        context.RegisterImplementationSourceOutput(
            ...
            (ctx, item) =>
            {
                ...
                GeneratorContext generatorContext = item.Item2;

                processAction(diagnostics.Value, generatorContext, ctx);
            });
    }
