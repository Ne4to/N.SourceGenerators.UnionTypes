...
readonly partial struct ReadonlyStructUnion
{
}

public class ReadonlyStructUnionTests
{
    [Fact]
    public void AllOperations()
    {
        ReadonlyStructUnion unionValue = "42";
        ...
        // .ctor
        ReadonlyStructUnion ctorValueNotSet = new ReadonlyStructUnion(new ValueNotSet());
        Assert.Equal(new ValueNotSet(), ctorValueNotSet);
        
        ReadonlyStructUnion ctorString = new ReadonlyStructUnion("42");
        Assert.Equal("42", ctorString);
        
        ReadonlyStructUnion ctorUserStatus = new ReadonlyStructUnion(UserStatus.Active);
        Assert.Equal(UserStatus.Active, ctorUserStatus);
        
        // implicit operator
        ReadonlyStructUnion implicitValueNotSet = new ValueNotSet();
        Assert.Equal(new ValueNotSet(), implicitValueNotSet);
        
        ReadonlyStructUnion implicitUserStatus = UserStatus.Active;
